...
class ConanFileUpdater:
    ...
    def make_all_packages_check_all_remotes(self):
        for p in self.packages:
            p.use_all_remotes = True
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Conan Dependabot')

    parser.add_argument("--conanfiles", type=str,
                        help="The conanfiles to check")
    parser.add_argument("--include-cmake-files", default=False,
                        action='store_true',
                        help="Include the cmake files")

    parser.add_argument("--force-versions", type=str, help="versions")
    ...
    conanfiles = None
    if args.conanfiles:
        conanfiles = [root_dir / Path(x.strip()) for x in
                      args.conanfiles.replace(r'\n', ',').split(',')]

    else:
        conanfiles = locate_conan_files(
            base_dir=root_dir,
            include_cmake_files=args.include_cmake_files)

    force_versions = []
    if args.force_versions:
        for x in args.force_versions.replace(r'\n', ',').split(','):
            package, v = x.split('=')
            force_versions.append((package.strip(), v.strip()))

    print(f"{force_versions=}")
    ...
    for conanfile in conanfiles:
        ...
        # Make all packages check all remotes
        conanfileupdater.make_all_packages_check_all_remotes()

        for p, v in force_versions:
            conanfileupdater.force_package_version(
                package_name=p, version_contains=v
            )
